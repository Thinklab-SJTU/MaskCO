...
def train_tsp(
    ...
    def train_step(
        ...
        tgt_adjmat = sol2adj(target, dtype=jnp.float32)
        current = target
        keep_prob = timestep
        ...
        mask = jax.random.bernoulli(
            ...
            shape=(batch_size_per_device, num_nodes),
        )
        cur_adjmat = sol2adj_with_mask(current, mask=mask, dtype=jnp.float16)
        def loss_fn(params: nnx.State):
            ...
            return - (logits * tgt_adjmat).mean() * (num_nodes / 2)     # Should the model be required to predict the known part? 
    ...
    while True:
        ...
        for coords, _, target, timestep in dataloader:
            step += 1

            if is_replicated:
                coords, target, timestep = tuple(map(
                    lambda x: x.reshape((num_devices, batch_size_per_device) + x.shape[1:]),
                    (coords, target, timestep),
                ))
            
            loss, params, opt_state, key = train_step(graphdef, params, opt_state, coords, target, timestep, key)
